...
//     try {
//       const q = query(collection(db, 'Newavis'), orderBy('timestamp', 'desc'));
...
//       await addDoc(collection(db, 'Newavis'), {
//         name,
//         email,
//         message,
//         timestamp: new Date()
...
//   const fetchAvis = async () => {
//     try {
//       const q = query(collection(db, 'Newavis'), where('is_confirmed', '==', true), orderBy('timestamp', 'desc'));
//       const querySnapshot = await getDocs(q);
//       const avisList = querySnapshot.docs.map(doc => ({
//         id: doc.id,
//         ...doc.data(),
...
//         email,
//         message,
//         is_confirmed: false, // Set is_confirmed to false by default
//         timestamp: new Date()
//       });
//       alert('Avis envoyé avec succès');
//       setFormSuccess(true);
...
// export default FeedbackComponent;
import React, { useState, useEffect } from 'react';
import { collection, addDoc, getDocs, query, where, orderBy } from 'firebase/firestore'; // Import where and orderBy
import { db } from '../components/auth/firebase';
import { Modal, Button, Form } from 'react-bootstrap';
import '../assets/css/style.css';
import '../assets/css/FeedbackComponent.css';
...
const FeedbackComponent = () => {
  ...
  const fetchAvis = async () => {
    try {
      const q = query(collection(db, 'avis'), where('is_confirmed', '==', true), orderBy('timestamp', 'desc')); // Correctly use where and orderBy
      const querySnapshot = await getDocs(q);
      const avisList = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
