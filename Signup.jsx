...
export default function Signup({ onSignup }) {
    ...
    // Gestion de la vérification du CAPTCHA
    ...
    const handleSubmit = (event) => {
        ...
        if (Object.keys(errors).length === 0) {
            axios.post("http://localhost:8002/site_location", values)
                .then(res => {
                    console.log("Registered Successfully!");
                    setApiError("");
                    if (onSignup) onSignup();
                    alert("Inscription réussie !");
                    // Fermer la popup et rediriger ou afficher un message de succès ici
                })
                .catch(err => {
